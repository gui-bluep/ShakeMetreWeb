...
- **The application's root goes to the dashboard, never to Breeze's Welcome page — and it takes a global middleware, not just a route.** Decided with the user: ShakeMetre is an internal tool and Laravel's landing page serves nobody. The guard is untouched, because the redirect targets `/dashboard`: a visitor who is not logged in lands on `/login` exactly as if they had asked for the dashboard. Verified in production, `/metre/` → `/metre/dashboard` → `/metre/login`, 200. `routes/web.php` says it (`Route::redirect('/', '/dashboard')->name('home')`) but `RedirectRootToDashboard` is what does it, because **under a URL prefix the root is not routable at all**: `CompiledRouteCollection::requestWithoutTrailingSlash()` strips the trailing slash from `REQUEST_URI` without regard for the base path, so `/metre/` becomes `/metre`, whose application path is empty; the compiled matcher finds nothing, the fallback covers only uncompiled routes, and Laravel ends up answering **405 Method Not Allowed, Supported methods: HEAD** — the requested verb being the one it excludes when it looks for alternates. A global middleware runs *before* routing, which is the only place an unroutable URL can be answered from. Three conditions are needed, which is why it only ever showed in production: a prefix, a trailing slash, and the route cache that `artisan optimize` installs at deploy time. Only the root is affected — `/metre/references/` reduces correctly to `/references`. Diagnosed by running the kernel on the exact request rather than by reading: the trace named `getRouteForMethods()` in one shot, after two wrong hypotheses (the closure route, then the route cache alone) had each been eliminated by measurement.
...
- **`APP_URL` is pinned in `phpunit.xml`, and that is not tidiness.** The test client prefixes every relative URI with `config('app.url')`, so on the production server — mounted at `https://fms99.mycloud.fm/metre` — every route test asked the router for `/metre/login` and got a 404: the prefix normally arrives through `SCRIPT_NAME`, which the test client does not fabricate. **415 of 616 tests failed on the server and none on a development machine.** The suite must not depend on where the application happens to be deployed.
- **Running the suite on the server needs `composer install` first**, dev packages being removed by `deploy/deploy.sh` (`--no-dev`). Restore `--no-dev` afterwards, or the next deploy will. Tests run on in-memory SQLite (`phpunit.xml`), so the production database is never touched — checked on the live one after a full run: 24 tables, 23 migrations, the three real accounts untouched.
