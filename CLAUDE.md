...
- **The Tags/Lots switch.** Lots is fully wired; Tags still shows `METL::tag1` read-only. What the source says, read since: `TAG_Tags` is **per métré** (`zkf_MET`) and holds `TagCode` + `TagText`, so a métré defines its own tag list; a line carries two, `TAG1` and `TAG2`; `MET::Sort_OrderTags` (1 or 2) picks which is primary, via `TAG_Choice1_cU` / `TAG_Choice2_cU` — that is the second sort flavour listed below; `METL::zg_TagsShow_b` looks like the switch itself; and `METL_Tag_AssignToSelection` assigns both to a selection with `Replace Field Contents`, exactly like the lot, refusing on a locked métré. What is still unknown: whether `TAG1` stores the code or the text, whether a tag must come from the métré's list or is free text, whether the switch shows one column or two, whether it also re-sorts, and where `Sort_OrderTags` is set. There is also no screen to define a métré's tags, which assigning them would need first.
