...
- **Documents are printed, not stored.** Established against the live server, not the export: the hosted file has **no document table** (`DEV/Raw/*` covers CART, CAT, CATS, JCARTMAT, LOT, MAT, MET, METC, METL, PRJ, REF, REFS, REFSL, SUP, TAG, ZSET, ZSTRI, ZVAR — nothing else), no container field, and instead a `METL_Print` layout folder whose members are the seven rows this page lists: `METL_ClientBudgetSummary_Print`, `METL_ClientBudgetSummaryBasic_Print`, `METL_ClientBudgetSimplifiedComposition_Print`, `METL_SupplierBudgetPurchase_Print`, `METL_SupplierBudgetOrdered_Print`, `METL_MetreComplete_List_PurchasesSales_Print`, plus `METT_SupplierTenderProcess_Print` / `METT_AllSuppliersTenderProcess_Print` under the tender folder. So a "document" is a PDF rendered on demand from a layout over the métré's lines — there is nothing to migrate, only to re-render. What each one contains still has to be read layout by layout (the export carries no layout detail), or supplied as example PDFs.
- **The métré page's `Fournisseur` card is the lots-with-their-supplier breakdown.** `MET_Form` in the live file carries a portal over the *project's* lots showing `Code`, `Title_FR` and `CPY_Name_ae` (the supplier company copied onto the lot), with this métré's `zsm_SumTotalBuy_noOptions` / `zsm_SumTotalOrdered_noOptions` per lot, plus three métré-level figures: `Tot_LotAssignedBuy_cU` = `Sum ( METL::LOT_AmountAssignedBuy )` where that per-line field is `Case ( not IsEmpty ( zkf_LOT ) ; PriceTotalBuy_noOptions_c ; 0 )`, its `NotAssigned` mirror on `IsEmpty ( zkf_LOT )`, and `Tot_LotAssignedOrdered_cU` on the ordered total. Note the gate is "the line **has a lot**", not "its lot has a supplier company" — unlike `GainOnPurchases_c`. Every input is already local (`lots.cpy_name_ae` is a column), so the card needs no ShakeDesign call.
- **`Nouvelle offre client`** on the métré page is a disabled button tagged `Bientôt`; `ShakeDesignClient::createOffer()` exists and is tested but nothing calls it yet. `MET_OFF_CreateClientOffer` is the source script to read. Listing a métré's existing offers has to go through **ShakeDesign's** own `API_OFF` layout (confirmed by the user to exist): `MET_Form` does carry a `met_OFF__Offers` portal, but its fields read `<No Access>` from the ShakeMetre account — OFF lives in ShakeDesign as an external data source, so the ShakeMetre Data API cannot be used to learn its shape. Opening one offer in the FileMaker client is **paused** by decision, and hits the same unknown as `SOR_GoTo` (target file, script name, parameter format).
